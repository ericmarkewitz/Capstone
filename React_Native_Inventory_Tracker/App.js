...
function Canning({ navigation }) {
  ...
  db.transaction((tx) => {
    tx.executeSql(
      'SELECT batchID, product, datePlaced, expDate, notes, quantity FROM Batch ORDER BY datePlaced;',
      [],
      (tx, results) => {
        for (var i = 0; i < results.rows.length; i++) {
  ...
  db.transaction((tx) => {
    tx.executeSql(
      'SELECT batchID, product, datePlaced, expDate, notes, quantity FROM Batch ORDER BY expDate;',
      [],
      (tx, results) => {
        for (var i = 0; i < results.rows.length; i++) {
